...
# Make spatial
# us <- cism::us
us$x <- us$lng <- us$lon <- us$xx <- us$longitude <- us$gpc_lng
us$y <- us$lat <- us$yy <- us$latitude <- us$gpc_lat
us <- us[,!grepl('X', names(us))]
names(us)[1:2] <- c('pos_adm_health_facility', 'loc_health_facility')
coordinates(us) <- ~gpc_lng+gpc_lat
# us <- us[!is.na(us$x),]
# coordinates(us) <- ~xx+yy
...
# JANUARY 2017
# REMOVE THE FOLLOWING PROBLEMATIC CLUSTER
# remove_these <- c(58)#c(119, 47, 58)
# clusters <- clusters %>% filter(!cluster %in% remove_these)
# randomly select
...
save(clusters, file =  'selected_clusters.RData')
