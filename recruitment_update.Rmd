...
# Add in the number of cases, etc.
or_df$positive <- 
...
The below is a table of the raw numbers.

```{r}
table(ifelse(pcd$tdr_res == 'Positive', 'Positive TDR', 'Negative TDR'), pcd$spray_status)
```
...
The **annual extrapolated incidence** (under 5) is `r overall_incidence` infections per 1,000 children-years (not taking into account seasonality).
...
The **annual extrapolated incidence** (under 5) per 1,000 children-years (not taking into account seasonality) is below. (Population figures need to be adjusted)
...
risk_map <- function(data = acd,
  ...
  g <-
    ...
    theme_cism() +
    labs(title = '',
         subtitle = 'Interpolated average weekly positivity ',
         x = 'Longitude',
         y = 'Latitude')
  return(g)
}
...
acd$acd_visit_date[acd$acd_visit_date < '2015-01-01'] <-
  acd$acd_visit_date[acd$acd_visit_date < '2015-01-01'] + 
  lubridate::years(20)
